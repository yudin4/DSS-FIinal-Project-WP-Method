...
<html lang="en">
  ...
  <body>
    ...
    <table id="tabelalt" class="table table-hover table-bordered" style="width:100%; text-align:center; border-color:#052f6e">
        ...
        <tbody>
            <?php
                    $select = mysqli_query($connect,"SELECT * FROM data ORDER BY id_data ASC");
                    $i=1;
                    while ($show = mysqli_fetch_assoc($select)){
            ?>
            <tr >
                            <td ><?php echo $show ['id_data'];?></td>
                            <td ><?php echo $show ['id_alternatif'];?></td>
                            <td ><?php echo $show ['id_kriteria'];?></td>
